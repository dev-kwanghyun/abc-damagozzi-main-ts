...
type StateType = "run" | "walk" | "idle" | "followMouse" | "sleep";
type imageType = "run" | "walk" | "idle" | "sleep";
class Character {
  ...
  imageUrlObj: Record<imageType, string> = {
    walk: "images/red_walk_8fps.gif",
    sleep: "images/red_sleep_8fps.gif",
    run: "images/red_run_8fps.gif",
    idle: "images/red_idle_8fps.gif",
  };
  ...
  clearAllTimers() {
    this.timers.forEach((timerId) => clearTimeout(timerId as number));
    this.timers.clear();
  }
  ...
  changeImageTo(imageName: imageType) {
  ...
  // 마우스 따라가는 함수...
  followMouseToPosition(clickX: number, state: StateType) {
    this.clearAllTimers();
    this.setState(state);
